...
def read_data_config(ids=None):
    ...
    return pd.DataFrame(columns=config_columns,
                        data=controller_connector.read(table="file_configs", columns=', '.join(config_columns),
...
def read_data_config_join_log(ids=None, file_configs_columns=None, file_logs_columns=None,
    ...
    file_configs_columns = config_columns if file_configs_columns is None or file_configs_columns == [] else file_configs_columns

    file_logs_columns = log_columns if file_logs_columns is None or file_logs_columns == [] else file_logs_columns

    file_configs_columns = ['fc.' + text for text in file_configs_columns]
    file_logs_columns = ['fl.' + text for text in file_logs_columns]

    columns = np.concatenate((file_configs_columns, file_logs_columns))
    data = controller_connector.read(
        custom_query=f"select {', '.join(columns)} \
                                                           from data_warehouse_control.file_configs as fc left join data_warehouse_control.file_logs fl on fc.id = fl.file_config_id \
                                                           where fl.date = '{date.strftime('%Y-%m-%d')}' {';' if ids is None else ' and fc.id in ' + tuple(ids) + ';'}",
    )
