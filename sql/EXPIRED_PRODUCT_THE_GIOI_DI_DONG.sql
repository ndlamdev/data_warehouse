DROP PROCEDURE IF EXISTS data_warehouse_control.EXPIRED_PRODUCT_THE_GIOI_DI_DONG;

DELIMITER //

CREATE PROCEDURE data_warehouse_control.EXPIRED_PRODUCT_THE_GIOI_DI_DONG(OUT expired BIT,
...
BEGIN
    ...
    FROM (data_warehouse_staging.products_the_gioi_di_dong_staging AS p_t
        LEFT JOIN data_warehouse_staging.product_the_gioi_di_dong_images_staging AS img_t
        ON p_t.id = img_t.product_id
        LEFT JOIN data_warehouse_staging.product_the_gioi_di_dong_prices_staging AS pr_t
